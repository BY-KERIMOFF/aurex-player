...
function getDeviceMAC() {
    // LG webOS Native ID Detection
    if (window.webOSDev && window.webOSDev.LGUDID) {
        window.webOSDev.LGUDID({
            onSuccess: (res) => {
                if (res.id) {
                    state.mac = normalizeMac(res.id);
                    localStorage.setItem('aurex_mac', state.mac);
                }
            }
        });
...
function normalizeKey(e) {
    if (e.keyCode === 461 || e.key === 'MediaStop' || e.key === 'Escape') return 'Backspace';
    if (e.keyCode === 37 || e.key === 'ArrowLeft') return 'ArrowLeft';
    if (e.keyCode === 38 || e.key === 'ArrowUp') return 'ArrowUp';
    if (e.keyCode === 39 || e.key === 'ArrowRight') return 'ArrowRight';
    if (e.keyCode === 40 || e.key === 'ArrowDown') return 'ArrowDown';
    if (e.keyCode === 13 || e.key === 415 || e.key === 'Enter') return 'Enter';
    if (e.key === 'AudioVolumeUp' || e.keyCode === 447) return 'AudioVolumeUp';
    if (e.key === 'AudioVolumeDown' || e.keyCode === 448) return 'AudioVolumeDown';
    return e.key;
}
...
function handleDashboardNav(key) {
    if (key === 'Backspace') {
        // LG webOS System Exit Call
        if (window.webOS && window.webOS.platformBack) {
            window.webOS.platformBack();
        } else if (confirm("Tətbiqdən çıxmaq istəyirsiniz?")) {
            window.close();
        }
        return;
    }
